...
def generate_post_image(spec: PostSpec) -> bytes:
    ...
    if len(h_lines) > 3:
        ...
        h_lines = h_lines[:3]
    sub_lines = (
        _wrap(spec.subheadline, sub_font, max_text)[:2] if spec.subheadline else []
    )

    content_h = (
        len(h_lines) * h_lh + div_gap + div_h + div_gap + len(sub_lines) * sub_lh
    )
    ...
    cap_lines = (
        _wrap(spec.caption, cap_font, int(max_text * 0.75))[:2] if spec.caption else []
    )
