...
def chunk_markdown(text: str, source: str) -> List[Dict[str, Any]]:
    ...
    def flush(header: str, body: str) -> None:
        ...
        if len(combined) <= 500:
            chunks.append(
                _make_chunk(source, combined, {"header": header, "type": "markdown"})
            )
        else:
            ...
            if window.strip():
                chunks.append(
                    _make_chunk(
                        source, window.strip(), {"header": header, "type": "markdown"}
                    )
                )
...
def chunk_logs(text: str, source: str) -> List[Dict[str, Any]]:
    ...
    diag_timestamp_pattern = re.compile(
        ...
        if len(content) > 1500:
            ...
            for line in lines:
                if len(current) + len(line) + 1 > 1500:
                    chunks.append(
                        _make_chunk(source, current.strip(), {"type": "code"})
                    )
                    current = line + "\n"
                else:
                    current += line + "\n"
